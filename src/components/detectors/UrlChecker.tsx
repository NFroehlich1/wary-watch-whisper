...
const UrlChecker = () => {
  ...
  return <div className="space-y-4">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">Verify if a URL is secure
      </p>
      </div>
      
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex space-x-2">
          <Input type="url" placeholder="Enter URL (e.g., https://example.com)" value={url} onChange={handleUrlChange} className="flex-1" required />
          <Button type="submit" disabled={loading || !url.trim()}>
            {loading ? 'Checking...' : 'Check URL'}
          </Button>
        </div>
        
        {showHttpWarning && <Alert className="bg-yellow-50 border-yellow-200">
